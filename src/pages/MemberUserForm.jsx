...
const [formData, setFormData] = useState({
  ...
  const [directors, setDirectors] = useState([
    { director_name: "", director_pan_no: "", director_aadhar_no: "", director_gender: "",
      director_dob: "", user_pan_doc: null, user_addhar_doc: null }
  ]);
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 1) {
      ...
      ["name","mobile_no","email","business_mcc","company_type","company_pan_no","company_gst_no","cin_llpin","date_of_incorporation"].forEach(f=>{
        const val=formData[f];
        if(!val) newErrors[f]="Required";
        else {
          if(f==="name" && !nameRegex.test(val)) newErrors[f]="Invalid Name";
          if(f==="mobile_no" && !phoneRegex.test(val)) newErrors[f]="Invalid Mobile";
          if(f==="email" && !emailRegex.test(val)) newErrors[f]="Invalid Email";
          if(f==="business_mcc" && !textNumberRegex.test(val)) newErrors[f]="Invalid MCC";
          if(f==="company_pan_no" && !textNumberRegex.test(val)) newErrors[f]="Invalid PAN";
          if(f==="company_gst_no" && !textNumberRegex.test(val)) newErrors[f]="Invalid GST";
          if(f==="cin_llpin" && !textNumberRegex.test(val)) newErrors[f]="Invalid CIN/LLPIN";
        }
      });
      ["company_pan_no_doc","company_gst_no_doc","cancel_cheque_doc"].forEach(f=>{
    ...
    if(currentStep===3){
      ["city","state","district","pin_code","address"].forEach(f=>{
        const val=formData[f];
        if(!val) newErrors[f]="Required";
        if(f==="pin_code" && val && !pinnumberRegex.test(val)) newErrors[f]="Invalid Pincode";
      });
    }

    if(currentStep===4){
      directors.forEach((d,i)=>{
        ["director_name","director_pan_no","director_aadhar_no"].forEach(f=>{
          const val=d[f];
          if(!val) newErrors[`${f}_${i}`]="Required";
          if(f==="director_name" && val && !nameRegex.test(val)) newErrors[`${f}_${i}`]="Invalid Name";
          if(f==="director_aadhar_no" && val && !aadharRegex.test(val)) newErrors[`${f}_${i}`]="Invalid Aadhaar";
        });
        if(!d.user_pan_doc) newErrors[`user_pan_doc_${i}`]="PAN File required";
        if(!d.user_addhar_doc) newErrors[`user_addhar_doc_${i}`]="Aadhaar File required";
      });
    }
  ...
  const handleFileChange=(key,file)=>setCompanyDocs(prev=>({...prev,[key]:file}));
  ...
  const handleSubmit=async()=>{
    ...
    Object.keys(companyDocs).forEach(k=>companyDocs[k] && fd.append(k,companyDocs[k]));
    directors.forEach((d,i)=>Object.entries(d).forEach(([k,v])=>v && fd.append(`director_info[${i}][${k}]`,v)));
...
{["business_mcc","company_type","company_pan_no","company_gst_no","cin_llpin","date_of_incorporation","website_url"].map((key,index)=>(
    ...
    {errors[key] && <p className="text-red-600 text-sm">{errors[key]}</p>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            {["company_pan_no_doc","company_gst_no_doc","cancel_cheque_doc"].map((key)=>
              <div key={key}>
                <label className="block mb-1 font-medium">{key.replace(/_/g," ").replace("doc","").trim()} *</label>
                <input type="file" className={input} onChange={e=>handleFileChange(key,e.target.files[0])} />
                {errors[key] && <p className="text-red-600 text-sm">{errors[key]}</p>}
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {["account_holder_name","bank_account_no","ifsc_code"].map((key)=>
              <div key={key}>
                <label className="block mb-1 font-medium">{key.replace(/_/g," ")} {key!=="ifsc_code"?"*":""}</label>
                <input className={input} value={formData[key]} onChange={e=>handleChange(key,e.target.value)} />
                {errors[key] && <p className="text-red-600 text-sm">{errors[key]}</p>}
              </div>
            )}
            ...
            <div key={i} className="p-4 border rounded-lg bg-white border border-gray-200 shadow-sm rounded-xl space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {["director_name","director_pan_no","director_aadhar_no","director_gender","director_dob"].map((k)=>
                  <div key={k}>
                    {k==="director_gender" ? (
                      <select className={input} value={d[k]} onChange={e=>handleDirectorChange(i,k,e.target.value)}>
                      ...
                      <input type="date" className={input} value={d[k]} onChange={e=>handleDirectorChange(i,k,e.target.value)} />
                    ) : (
                      <input placeholder={k.replace(/_/g," ")} className={input} value={d[k]} onChange={e=>handleDirectorChange(i,k,e.target.value)} />
                    )}
                    {errors[`${k}_${i}`] && <p className="text-red-600 text-sm">{errors[`${k}_${i}`]}</p>}
                  </div>
                )}
                {["user_pan_doc","user_addhar_doc"].map((k)=>
                  <div key={k}>
                    <input type="file" className={input} onChange={e=>handleDirectorChange(i,k,e.target.files[0])} />
                    {errors[`${k}_${i}`] && <p className="text-red-600 text-sm">{errors[`${k}_${i}`]}</p>}
                  </div>
                )}
              </div>
  ...
  <div className="space-y-6">
    ...
    <div className="border rounded-lg p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
      ...
      <p><b>Business MCC:</b> {formData.business_mcc}</p>
      <p><b>Company Type:</b> {formData.company_type}</p>
      <p><b>PAN:</b> {formData.company_pan_no}</p>
      <p><b>GST:</b> {formData.company_gst_no}</p>
      <p><b>CIN / LLPIN:</b> {formData.cin_llpin}</p>
      <p><b>Date of Incorporation:</b> {formData.date_of_incorporation}</p>
      <p><b>Website:</b> {formData.website_url || "-"}</p>
    </div>
    ...
    <div className="border rounded-lg p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
      ...
      <p><b>IFSC Code:</b> {formData.ifsc_code || "-"}</p>
    </div>
    ...
    <div className="border rounded-lg p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
      ...
      <p><b>Pincode:</b> {formData.pin_code}</p>
